...
const fullText1 = splitLines("Gửi Cục Dàng Của Bụp", 3);
const fullText2 = splitLines("CHÚC MỪNG VALENTINE ĐẦU TIÊN MÌNH ĐƯỢC YÊU THƯƠNG CÙNG NHAU ❤️ ", 3);
const fullText3 = splitLines("Từ giờ trên đoạn đường phía trước, em không còn cô đơn bước đi một mình nữa", 3);
const fullText4 = splitLines(" Mà sẽ có anh cùng em vượt qua những lúc yếu mềm nhất cũng như là hạnh phúc nhất. Anh mong dù có chuyện gì đi nữa thì anh và em vẫn nắm chặt tay nhau nha!!!", 3);
const fullText5 = splitLines("Anh Yêu Cục Dàng của Anh nhiều  ❤️ !!!", 4);

const allTexts = [fullText1, fullText2, fullText3, fullText4, fullText5];
...
const startY = sectionTop + (window.innerHeight - (lines.length * lineHeight)) / 2 + (lineHeight / 2);  targetDotsQueue = [];
...
function startShow() {
  ...
  animate();
  document.getElementById("bear").classList.add("bear-active");
}
